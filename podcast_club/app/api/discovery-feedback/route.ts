...
async function getMemberReactions(memberId: string) {
  const rows = await DiscoveryFeedbackModel.find({ member: memberId }).select('recommendationKey reaction').lean();
  return Object.fromEntries(rows.map((row) => [row.recommendationKey, row.reaction]));
}
...
export async function GET() {
  ...
  return NextResponse.json({ reactions: await getMemberReactions(session.member._id) });
}
...
export async function PUT(request: Request) {
  ...
  if (body?.reaction === null) {
    ...
    await DiscoveryFeedbackModel.findOneAndUpdate(
      ...
      { $set: { title, href, reaction: body?.reaction } },
