...
export async function GET() {
  try {
    ...
    const publicRound = poolState.r16.status === "open" || poolState.r16.status === "locked";
    const matches = publicRound
      ? await MatchModel.find({ poolSlug: defaultPoolSlug, stage: "r16" }).sort({ order: 1 }).lean()
      : [];
    ...
    console.error("Poolarama /api/r16 failed", error);
