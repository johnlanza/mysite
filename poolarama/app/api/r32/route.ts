...
export async function GET() {
  try {
    ...
    const publicRound = poolState.r32.status === "open" || poolState.r32.status === "locked";
    const matches = publicRound
      ? await MatchModel.find({ poolSlug: defaultPoolSlug, stage: "r32" }).sort({ order: 1 }).lean()
      : [];
    ...
    console.error("Poolarama /api/r32 failed", error);
